...
All implementation in this single file - not really valuable.
...
def place(board, player: int, position: tuple):
    if (player < 1) or (player > 2):
        raise Exception("WrongPlayerNumberException")
    (row, col) = position
    if board[row, col] == 0:
...
posspos = possibilites(game_board)
